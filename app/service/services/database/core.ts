...
export class DatabaseCore extends Eventable<IDatabaseCoreState> {
  ...
  async initRealm(reinit = false): Promise<Realm> {
    ...
    const lastDBVersion = (await PLMainAPI.preferenceService.get(
      "lastDBVersion"
    )) as number;
    ...
    if (type === ConfigType.Cloud) {
      try {
        ...
        await new Promise((resolve) => {
          this._realm!.write(() => {
            syncMigrate(this._realm!, this.getPartition(), lastDBVersion);
            resolve(true);
          });
        });
      } catch (err) {
    ...
    await PLMainAPI.preferenceService.set({
      lastDBVersion: DATABASE_SCHEMA_VERSION,
    });
  ...
  async getConfig(): Promise<{
    ...
    if (await PLMainAPI.preferenceService.get("useSync")) {
      try {
        syncPassword =
          (await await PLMainAPI.preferenceService.getPassword("realmSync")) ||
          "";
      } catch (err) {
        this._logService.error(
      ...
      if (
        (await PLMainAPI.preferenceService.get("syncEmail")) !== "" &&
        syncPassword
      ) {
        return {
  ...
  private async _loginCloud(): Promise<Realm.User | null> {
    if (!this._app) {
      ...
      const id = (await PLMainAPI.preferenceService.get("syncAPPID")) as string;
    ...
    try {
      ...
      const credentials = Realm.Credentials.emailPassword(
        (await PLMainAPI.preferenceService.get("syncEmail")) as string,
        syncPassword
      );
